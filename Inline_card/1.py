...
from users_base import get_random_anime_sql, search_anime_base
...
async def check_subscription(user_id):
    ...
    if not channels:
        return True, []  # Agar majburiy kanallar bo'lmasa, ruxsat beriladi
    not_subscribed = []
            ...
            for a in anime_results:
                ...
                buttons = InlineKeyboardMarkup().add(
                    InlineKeyboardButton(
                        text="✨Tomosha qilish✨",
                        url=f"https://t.me/{a1}?start={anime_id}serie"
                    )
                )
                results.append(
                    InlineQueryResultArticle(
